...
static int audiosocket_run(struct ast_channel *chan, const char *id, int svc)
{
	...
	while (1) {
		...
		if (targetChan) {
			...
			/* Backport AudioSocket DTMF framing for the phone bridge.
			 * Upstream Asterisk 20.6 only forwards AST_FRAME_VOICE.
			 * Protocol: type 0x03, big-endian length 1, ASCII digit. */
			if (f->frametype == AST_FRAME_DTMF_END) {
				char digit_message[4] = {0x03, 0x00, 0x01, (char) f->subclass.integer};
				if (ast_carefulwrite(svc, digit_message, sizeof(digit_message), 100)) {
					ast_frfree(f);
					return -1;
				}
			}
